...
function test_os()
{
    var fd, fpath, fname, fdir, buf, buf2, i, files, fdate, st, link_path;
    ...
    files = os.readdir(fdir);
    assert(files.indexOf(fname) >= 0);

    fdate = 10000;

    os.utimes(fpath, fdate, fdate);
...
function test_async_promise_rejection()
{
    ...
    p2.catch(() => counter++);
    p3.then(() => counter++)
    setTimeout(() => { assert(counter, 3) }, 10);
}
