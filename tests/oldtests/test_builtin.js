...
function throw_error(msg) {
  if (throw_errors) throw Error(msg);
  console.log(msg);
  status = 1;
}
...
function assert(actual, expected, message) {
  ...
  throw_error(
    "assertion failed: got " +
      get_full_type(actual) +
      ":|" +
      actual +
      "|, expected " +
      get_full_type(expected) +
      ":|" +
      expected +
      "|" +
      (message ? " (" + message + ")" : "")
...
function check_error_pos(e, expected_error, line_num, col_num) {
  ...
  if (expected_error === SyntaxError) expected_pos += "\n";
  else expected_pos += ")";
  if (e.stack.indexOf(expected_pos) < 0) {
    throw_error(
      "unexpected line or column number. error=" +
        e.message +
        ".got |" +
        e.stack +
        "|, expected |" +
...
function test_date() {
  ...
  assert(new Date("Jan 1 2000").toISOString(), d.toISOString());
  assert(new Date("Jan 1 2000 00:00").toISOString(), d.toISOString());
  assert(new Date("Jan 1 2000 00:00:00").toISOString(), d.toISOString());
  assert(
    new Date("Jan 1 2000 00:00:00 GMT+0100").toISOString(),
    "1999-12-31T23:00:00.000Z"
  );
  assert(
    new Date("Jan 1 2000 00:00:00 GMT+0200").toISOString(),
    "1999-12-31T22:00:00.000Z"
  );
  assert(new Date("Sat Jan 1 2000").toISOString(), d.toISOString());
  assert(new Date("Sat Jan 1 2000 00:00").toISOString(), d.toISOString());
  assert(new Date("Sat Jan 1 2000 00:00:00").toISOString(), d.toISOString());
  assert(
    new Date("Sat Jan 1 2000 00:00:00 GMT+0100").toISOString(),
    "1999-12-31T23:00:00.000Z"
  );
  assert(
    new Date("Sat Jan 1 2000 00:00:00 GMT+0200").toISOString(),
    "1999-12-31T22:00:00.000Z"
  );
  ...
  assert(
    new Date("2020-01-01T01:01:01.123Z").toISOString(),
    "2020-01-01T01:01:01.123Z"
  );
  /* implementation defined behavior */
  assert(
    new Date("2020-01-01T01:01:01.1Z").toISOString(),
    "2020-01-01T01:01:01.100Z"
  );
  assert(
    new Date("2020-01-01T01:01:01.12Z").toISOString(),
    "2020-01-01T01:01:01.120Z"
  );
  assert(
    new Date("2020-01-01T01:01:01.1234Z").toISOString(),
    "2020-01-01T01:01:01.123Z"
  );
  assert(
    new Date("2020-01-01T01:01:01.12345Z").toISOString(),
    "2020-01-01T01:01:01.123Z"
  );
  assert(
    new Date("2020-01-01T01:01:01.1235Z").toISOString(),
    "2020-01-01T01:01:01.123Z"
  );
  assert(
    new Date("2020-01-01T01:01:01.9999Z").toISOString(),
...
function test_generator() {
  ...
  function* f3() {
    var ret;
    /* test stack consistency with nip_catch to handle yield return +
     * finally clause */
    try {
      ret = 2 + (yield 1);
    } catch (e) {
...
function rope_concat(n, dir) {
  ...
  s = "";
  if (dir > 0) {
    for (i = 0; i < n; i++) s += String.fromCharCode(i & 0xffff);
  } else {
    for (i = n - 1; i >= 0; i--) s = String.fromCharCode(i & 0xffff) + s;
  }
