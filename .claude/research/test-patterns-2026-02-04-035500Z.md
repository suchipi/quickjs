...
For stable snapshots, leverage RunContext's cleanResult() method to replace absolute paths with `<rootDir>` in assertions when output may contain paths.
...
```typescript
expect(run.cleanResult().stdout).toContain("expected text");
```
