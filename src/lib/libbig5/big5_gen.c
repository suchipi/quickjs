/*
 * Big5 table generator
 *
 * Reads the WHATWG index-big5.txt file and generates a C header
 * containing:
 *   1. The decode table (pointer → Unicode)
 *   2. The encode table (Unicode → pointer) for encoding
 *
 * Usage: big5_gen <input_dir> <output_file>
 *
 * The input_dir should contain index-big5.txt
 */
...
int main(int argc, char **argv)
{
    ...
    snprintf(inpath, sizeof(inpath), "%s/index-big5.txt", argv[1]);
    ...
    if (max_pointer < 0) {
        fprintf(stderr, "big5_gen: no entries found in %s\n", inpath);
        return 1;
    }
    ...
    if (!decode_table || !encode_entries) {
        fprintf(stderr, "big5_gen: out of memory\n");
        free(decode_table);
        free(encode_entries);
        return 1;
    }
    ...
    fprintf(stderr, "big5_gen: read %d decode entries (max pointer %d), %d unique encode entries\n",
    ...
    fprintf(fout, "/* Auto-generated by big5_gen from index-big5.txt. Do not edit. */\n\n");

    /* Decode table */
    fprintf(fout, "#define BIG5_TABLE_SIZE %d\n\n", table_size);
    fprintf(fout, "static const unsigned int big5_decode_table[BIG5_TABLE_SIZE] = {\n");

    for (i = 0; i < table_size; i++) {
        if (i % 8 == 0)
            fprintf(fout, "    ");
        fprintf(fout, "0x%05X,", decode_table[i]);
    ...
    fprintf(fout, "#define BIG5_ENCODE_SIZE %d\n\n", encode_count);
    fprintf(fout, "typedef struct {\n");
    fprintf(fout, "    unsigned int codepoint;\n");
    fprintf(fout, "    unsigned int pointer;\n");
    fprintf(fout, "} Big5EncodeEntry;\n\n");
    fprintf(fout, "static const Big5EncodeEntry big5_encode_table[BIG5_ENCODE_SIZE] = {\n");

    for (i = 0; i < encode_count; i++) {
        if (i % 4 == 0)
            fprintf(fout, "    ");
        fprintf(fout, "{0x%05X, %5d},", encode_entries[i].codepoint, encode_entries[i].pointer);
