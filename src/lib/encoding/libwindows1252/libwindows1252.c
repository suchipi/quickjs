...
#include "libwindows1252-table.h"
...
uint32_t windows1252_decode(uint8_t byte)
{
    ...
    uint32_t cp = windows1252_decode_table[byte - 0x80];
...
{
    int lo = 0, hi = WINDOWS1252_ENCODE_SIZE - 1;

    while (lo <= hi) {
        int mid = (lo + hi) / 2;
        uint32_t mid_cp = windows1252_encode_table[mid].codepoint;
        if (mid_cp == codepoint)
            return windows1252_encode_table[mid].byte;
