...
/* Decode an EUC-KR double-byte sequence to a Unicode code point.
   lead: lead byte (0x81-0xFE)
   trail: trail byte (0x41-0xFE)
   Returns Unicode code point, or 0 if unmapped. */
uint32_t euckr_decode(uint8_t lead, uint8_t trail);

/* Encode a Unicode code point to EUC-KR.
   cp: Unicode code point
   out_lead: output lead byte
   out_trail: output trail byte
   Returns:
     0 if the code point cannot be encoded in EUC-KR
