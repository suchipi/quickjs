...
/*
 * On Windows, _pipe() creates file descriptors backed by anonymous pipe
 * HANDLEs. The event loop uses WaitForMultipleObjects on HANDLEs obtained
 * via _get_osfhandle(). Socket HANDLEs are more reliably waitable across
 * Windows versions, so we create a self-connected TCP socket pair instead.
 */
static int win32_socketpair(int fds[2]) {
    ...
    /* Convert sockets to C runtime file descriptors so they work with
     * read()/write()/close() and the event loop's fd-based tracking. */
    fds[0] = _open_osfhandle((intptr_t)server, _O_RDONLY);
    fds[1] = _open_osfhandle((intptr_t)client, _O_WRONLY);
    if (fds[0] < 0 || fds[1] < 0) {
        closesocket(server);
        closesocket(client);
        return -1;
...
#define pipe(fds) win32_socketpair(fds)
#endif /* _WIN32 */
...
static JSValue js_response_json(JSContext *ctx, JSValueConst this_val,
    ...
    ResponseData *rd = JS_GetOpaque2(ctx, this_val, js_response_class_id);
    if (!rd) return JS_EXCEPTION;
    /* Convert body to a JS string first, then use JSON.parse.
     * This avoids potential issues with JS_ParseJSON on non-null-terminated
     * buffers from the HTTP client. */
    JSValue str = JS_NewStringLen(ctx, (const char *)rd->http_resp->body,
                                  rd->http_resp->body_len);
    if (JS_IsException(str))
        return js_rejected_promise(ctx, JS_GetException(ctx));
    ...
    JSValue parsed = JS_Call(ctx, parse_fn, json_obj, 1, &str);
